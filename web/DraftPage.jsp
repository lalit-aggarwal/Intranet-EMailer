...
<%@ page import="java.sql.*" %>
    ...
    <body>
        <div>
            <form action="DraftDeleteController">
        ...
        try{
        PreparedStatement ps = con.prepareStatement("select * from Draft where user_id=?");
	...
	ResultSet rs = ps.executeQuery();
        ...
        <tr><td> <input type="checkbox" name="iid" value="<%=rs.getString(1)%>"  /></td><td><%=i%></td><td><a href='mainpage.jsp?id=<%=rs.getString(1)%>&name=draft'><%= rs.getString(2) %></a> </td><td><%=rs.getDate(4)%></td></tr>
